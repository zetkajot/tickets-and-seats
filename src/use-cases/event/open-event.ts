...
export default class OpenEvent extends UseCase<Input, Output> {
  async execute({ eventId }: Input): Promise<Output> {
    ...
    await this.saveOpenedEvent(event);
  ...
  private async findEvent(eventId: string): Promise<Event> {
    const eventData = <StoredEventData> (await tryFindingEntityData.customized({
      ...
      related: false,
      unique: true,
    })(this.dataVendor.findEvent, { id: eventId }))[0];
  ...
  private async saveOpenedEvent(event: Event): Promise<void> {
    const eventData = deconstructEvent(event);
    await tryExecutingStorageQuery(this.dataVendor.saveEvent, eventData);
  }
}
